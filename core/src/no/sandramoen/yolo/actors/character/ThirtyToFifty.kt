...
class ThirtyToFifty(x: Float, y: Float, s: Stage) : BaseActor(x, y, s) {
    private var tag = "ThirtyToFifty"
    ...
    init {
        ...
        for (i in 0 until 15)
            animationImages.add(BaseGame.textureAtlas!!.findRegion("character/30-50/head"))
        animationImages.add(BaseGame.textureAtlas!!.findRegion("character/30-50/head-blink"))
        head = BaseActor(0f, 0f, s)
        head.setAnimation(Animation(.5f, animationImages, Animation.PlayMode.LOOP))
        head.setSize(35f, 17f)
        head.setPosition(x - 16.8f, y - 1.0f)
        ...
        body.loadImage("character/30-50/body")
        body.setSize(25f, 25.86f)
        body.setPosition(x - 12f, y - 25f)

        leftArm = BaseActor(0f, 0f, s)
        leftArm.loadImage("character/30-50/leftArm")
        leftArm.setSize(10f, 12f)
        leftArm.setPosition(x + 10.4f, y - 12.5f)
        ...
        rightArm.loadImage("character/30-50/rightArm")
        rightArm.setSize(10f, 12f)
        rightArm.setPosition(x - 19f, y - 12.5f)
