...
class FifteenToTwenty(x: Float, y: Float, s: Stage) : BaseActor(x, y, s) {
    private var tag = "FifteenToTwenty"
    ...
    init {
        ...
        for (i in 0 until 15)
            animationImages.add(BaseGame.textureAtlas!!.findRegion("character/15-20/head"))
        animationImages.add(BaseGame.textureAtlas!!.findRegion("character/15-20/head-blink"))
        head = BaseActor(0f, 0f, s)
        head.setAnimation(Animation(.5f, animationImages, Animation.PlayMode.LOOP))
        head.setSize(28f, 18f)
        head.setPosition(x - 13.5f, y - 7.7f)
        ...
        body.loadImage("character/15-20/body")
        body.setSize(19.7f, 20.7f)
        body.setPosition(x - 9.3f, y - 25f)

        leftArm = BaseActor(0f, 0f, s)
        leftArm.loadImage("character/15-20/leftArm")
        leftArm.setSize(8.5f, 8.5f)
        leftArm.setPosition(x + 8.0f, y - 13.5f)
        ...
        rightArm.loadImage("character/15-20/rightArm")
        rightArm.setSize(8.5f, 8.5f)
        rightArm.setPosition(x - 15.0f, y - 13.4f)
